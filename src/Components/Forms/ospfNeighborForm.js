...
export function ModifyOspfNeighbor(props){
    ...
    return <Card bg={"dark"} style={{textAlign: 'left'}}>
                <Card.Body>
                    <Card.Title className="mb-4" style={{color: 'white', textAlign: "center"}}>Add OSPF Neighbor</Card.Title>
                        <Form ref={bgpForm} onSubmit={handleSubmit}>
                            <Row>
                                <Form.Group as={Col} controlId="network">
                                    ...
                                    <input  type="text" className="form-control input-text mb-3" onChange={e => setNetwork(e.target.value)} placeholder="Remote-as" name="Remote-as" style={{textAlign: 'left', fontWeight: 'bold' }}/>
                                </Form.Group>
                            </Row>
